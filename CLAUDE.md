...
> Share the GitHub repo URL or drop this zip into a new Claude conversation and say:
> **"Start a new project based on this."**
>
...
## Bootstrap from GitHub (Claude: read this first)

When starting from a GitHub URL or a fresh clone, follow these steps before
doing anything else:

1. **Clone the repo** (if not already on disk):
   ```bash
   git clone <repo-url> hubstaff-shell
   cd hubstaff-shell
   ```

2. **Generate the preview config**:
   ```bash
   python3 preview-setup.py
   ```
   This writes `.claude/launch.json` with an in-memory HTTP server that works
   inside the Claude Code preview sandbox (standard `python3 -m http.server`
   fails there because the sandboxed subprocess cannot call `getcwd()`).

3. **Start the preview** by calling `preview_start("hubstaff-shell")`.

4. **Confirm it renders** — take a screenshot. You should see the Hubstaff
   chrome with the placeholder card in the content area.

> **After every file edit**, re-run `python3 preview-setup.py` and restart
> the preview server so the in-memory snapshot reflects the latest changes.
...
Hubstaff app chrome with Design Annotations pre-wired. It is the **base layout**
for prototyping any Hubstaff page or feature. Nothing business-specific is included.
...
- **Design Annotations** pre-wired — right-click or the floating "Annotations" button opens the task drawer
...
├─ design-annotations.js           ← annotations engine (do not edit — fixed dependency)
├─ design-annotations.data.js      ← YOUR tasks: pages array + annotations array
├─ preview-setup.py                ← generates .claude/launch.json for Claude Code preview
...
| **Annotation tasks** | `design-annotations.data.js` — `pages` array + `annotations` array |
| Annotations engine | `design-annotations.js` — treat as a fixed dependency, never edit |
...
The annotations system is **pre-wired** in `index.html`. It loads after the shell
and reads tasks from `design-annotations.data.js`.

### How to open the drawer
- **Right-click** anywhere on the page, or
- Click the floating **"Annotations"** button (bottom-right), or
- Press **⌘/Ctrl+Shift+A**

### Adding tasks
Edit `design-annotations.data.js` only — never touch `design-annotations.js`.

1. Add a page to the `pages` array if building a new screen.
2. Add annotation objects to the `annotations` array — one per task.
3. Use the selector ladder: `#id` → `.semantic-class` → structural → `[data-annotate="x"]`.
   If no durable hook exists, add `data-annotate="x"` to the element in the markup.

### init() config (already set in index.html)
| Option | Value | Reason |
|---|---|---|
| `mount` | `#shell-content` | content area shifts when drawer opens |
| `topOffset` | `52` | height of the fixed top bar |
| `trigger` | `['contextmenu','button']` | right-click + floating button (works in iframes/preview) |
| `taskCenter` | `{ anchor: '#hs-topbar', position: 'after' }` | "Design tasks" button placed after the top bar |

### After adding a new page
When you add a new HTML file (e.g. `reports.html`), copy the three annotation
`<script>` tags from `index.html` into the new file so the drawer works there too.
